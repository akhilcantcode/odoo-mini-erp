...
export class ManufacturingController {
  ...
  start = async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      const result = await this.service.start(req.params.id, companyId, (req as any).user?.id);
      res.json(result);
    } catch (err: any) {
      if (err.statusCode) {
        return res.status(err.statusCode).json({ message: err.message });
  ...
  complete = async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      const result = await this.service.complete(req.params.id, companyId, (req as any).user?.id);
      res.json(result);
    } catch (err: any) {
      if (err.statusCode) {
        return res.status(err.statusCode).json({ message: err.message });
  ...
  confirm = async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      const result = await this.service.confirm(req.params.id, companyId, (req as any).user?.id);
      res.json(result);
    } catch (err: any) {
      if (err.statusCode) {
        return res.status(err.statusCode).json({ message: err.message });
  ...
  cancel = async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      const result = await this.service.cancel(req.params.id, companyId, (req as any).user?.id);
      res.json(result);
    } catch (err: any) {
      if (err.statusCode) {
        return res.status(err.statusCode).json({ message: err.message });
  ...
  /**
   * DELETE /:id — Delete a manufacturing order.
   */
  delete = async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      res.json({ message: 'Manufacturing order deleted successfully', result });
    } catch (err: any) {
      if (err.statusCode) {
        return res.status(err.statusCode).json({ message: err.message });
      }
