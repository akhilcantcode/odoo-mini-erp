    ...
    <div className="space-y-5 animate-fade-in">
      ...
      <div className="flex items-start justify-between">
        ...
        <div className="flex items-center gap-2">
          ...
          <Btn variant="secondary" size="sm" onClick={() => {
            ...
            const rows = items.map(i => [i.productName, i.onHandQty, i.reservedQty, i.freeQty, new Date(i.updatedAt).toLocaleString()]);
      ...
      {loading ? (
        ...
        filteredItems.length === 0 && items.length === 0 ? (
          ...
          <Card><EmptyState icon={<Search size={20} />} title="No results" description={`No products match "${searchQuery}"`} /></Card>
        ) : (
                    ...
                    return (
                      ...
                      >
                        ...
                        <td className="px-5 py-3.5">
                          <div className="flex items-center gap-3">
                            <div className={`w-9 h-9 rounded-lg flex items-center justify-center text-xs font-bold ${avatarColor} transition-transform duration-200 group-hover:scale-105 flex-shrink-0`}>
                              {initials}
                            </div>
                            <div>
                        ...
                        <td className="px-5 py-3.5 text-center">
                          <span className={`font-semibold tabular-nums ${
                            i.freeQty > 0 ? 'text-emerald-600' : i.freeQty === 0 ? 'text-red-500' : 'text-red-500'
                          }`}>
                            {i.freeQty}
                          </span>
                        </td>
        ...
        filteredLedger.length === 0 && ledger.length === 0 ? (
          ...
          <Card><EmptyState icon={<Search size={20} />} title="No results" description={`No ledger entries match "${searchQuery}"`} /></Card>
        ) : (
          <Card className="overflow-hidden">
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                ...
                <tbody className="divide-y divide-gray-50">
                  {filteredLedger.map((e, idx) => {
                    ...
                    return (
                      <tr key={e.id} className="hover:bg-sky-50/30 transition-colors duration-150 group" style={{ animationDelay: `${idx * 30}ms` }}>
                        <td className="px-5 py-3.5">
                          <div className="flex items-center gap-3">
                        ...
                        <td className="px-5 py-3.5"><LedgerTypeBadge type={e.type} /></td>
                        <td className="px-5 py-3.5 text-center">
                          <span className={`inline-flex items-center gap-1 font-semibold tabular-nums ${e.changeQty >= 0 ? 'text-emerald-600' : 'text-red-500'}`}>
                            {e.changeQty >= 0 ? '+' : ''}{e.changeQty}
                          </span>
                        </td>
                        <td className="px-5 py-3.5 text-gray-400 text-xs font-mono">{e.referenceId ? e.referenceId.slice(0, 8) + '…' : '—'}</td>
        ...
        <div className="flex items-center justify-between border-b border-gray-100 pb-3 flex-wrap gap-3">
          <div className="flex items-center gap-2">
            ...
            {selectedOrder.status === 'confirmed' && (
              <Btn variant="primary" size="sm" onClick={() => handleAction(selectedOrder.id, 'receive')} disabled={acting === selectedOrder.id}>
                Received
              </Btn>
            )}
      ...
      {showForm && (
        <Card className="p-5 animate-fade-in space-y-4">
          <form onSubmit={handleCreate} className="space-y-4">
            ...
            <div className="flex justify-end gap-2">
              <Btn type="button" variant="secondary" onClick={() => { setShowForm(false); setDraftItems([]); setVendor(''); setVendorAddress(''); setResponsiblePersonId(''); }}>Cancel</Btn>
        ...
        <div className="flex items-center justify-between border-b border-gray-100 pb-3 flex-wrap gap-3">
          <div className="flex items-center gap-2">
            ...
            {(selectedOrder.status === 'confirmed' || selectedOrder.status === 'draft') && (
              <Btn variant="primary" size="sm" onClick={() => handleAction(selectedOrder.id, 'start')} disabled={acting === selectedOrder.id}>
                Start
              </Btn>
            )}
            {selectedOrder.status === 'in_progress' && (
              <Btn variant="primary" size="sm" onClick={() => handleAction(selectedOrder.id, 'complete')} disabled={acting === selectedOrder.id}>
                Produce
              </Btn>
            )}
            {(selectedOrder.status === 'draft' || selectedOrder.status === 'confirmed' || selectedOrder.status === 'in_progress') && (
              <Btn variant="danger" size="sm" onClick={() => handleAction(selectedOrder.id, 'cancel')} disabled={acting === selectedOrder.id}>
                Cancel
              </Btn>
            )}
          </div>
                    ...
                    {selectedOrder.workOrders?.map((wo) => {
                      ...
                      return (
                        <tr key={wo.id} className="hover:bg-gray-50/50 transition">
                          ...
                          <td className="px-5 py-3.5 text-center text-gray-500 font-mono font-semibold">{wo.plannedDuration.toFixed(2)}</td>
        ...
        <Card className="p-5 animate-fade-in">
          <form onSubmit={handleCreate} className="space-y-4">
            <div className="grid grid-cols-1 sm:grid-cols-4 gap-3">
              ...
              <Select label="Assignee" value={assigneeId} onChange={(e: React.ChangeEvent<HTMLSelectElement>) => setAssigneeId(e.target.value)}>
            ...
            <div className="flex justify-end gap-2">
              <Btn type="button" variant="secondary" onClick={() => { setShowForm(false); setMoProductId(''); setMoQty(''); setScheduleDate(''); setAssigneeId(''); }}>Cancel</Btn>
                  ...
                  <tr key={o.id} className="hover:bg-sky-50/30 transition">
                    ...
                    <td className="px-5 py-3">
                      <div className="flex gap-1">
                        ...
                        {o.status === 'cancelled' && (
                          <span className="text-xs text-red-500 font-medium flex items-center gap-1"><X size={12} /> Cancelled</span>
                        )}
                      </div>
                    </td>
                  </tr>
